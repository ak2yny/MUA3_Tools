...
from lib.lib_nun import NUNO1, NUNO3, NUNO5, NUNV1
...
class G1MS:
    ...
    def getName(self, localID: int, global_to_oid: dict = {}) -> str:
        globalID = self.localIDToGlobalID[localID ^ 0x80000000 if not self.bIsInternal and localID >> 31 else \
                                          self.findLocalID(localID)]
        return global_to_oid[globalID] if globalID in global_to_oid else f'bone_{globalID}'
...
class G1MGVertexAttribute:
    ...
    def __post_init__(self):
        ...
        self.semantic = GLTF_Semantic[self.semantic] # Raises index out of range if not supported
...
class G1MGIndexBuffer:
    ...
    def bitwidth(self) -> int:
        return self.dataType // 8
    @property
    def npDataType(self) -> dtype:
        return dtype(f'uint{self.dataType}')
    @property
    def glTF_typ(self) -> str:
        return 'SCALAR'
    @property
    def glTF_acc(self) -> int:
        """Accessor Data Type (unsigned)"""
        # For signed, remove +1
        if self.dataType == 64: return 0
        return 5120 + self.dataType // 16 * 2 + 1
...
class G1MGVAFormat(Enum):
    ...
    def _missing_(cls, value):
        ...
        new._value_ = value
        return new
    def read(self, data: bytes, vb: G1MGVertexBuffer, start: int) -> ndarray:
        """Read and return bytes as np.array, according to the G1MGVAFormat and G1MGVertexBuffer."""
        # No guarantee that the stride size works, but using a loop is more than 100x slower, even if using unpack_from.
        assert(vb.stride % self.byte_count == 0)
        esz = vb.stride // self.byte_count
        if self == G1MGVAFormat.UNORM8x4:
            return (frombuffer(data, self.dtype, count=vb.count * esz, offset=vb.offset) / 255.0).reshape(vb.count, esz)[:,start:start + self.size]
        return frombuffer(data, self.dtype, count=vb.count * esz, offset=vb.offset).reshape(vb.count, esz)[:,start:start + self.size]
    ...
    def get_vb(self, submesh_vb_index: int, semantic: str, data: bytes, layer_threshold: int = -1):
        ...
        for a in self.vertexAttributeSets[submesh_vb_index].attributes:
            if a.semantic == semantic and a.layer > layer_threshold:
                ...
                # Note: G1MGVertexAttribute.offset is usually 0. Unknown if it corresponds to bytes, 1D (flat) or 2D if not 0.
                return (unpack_from(f'{E} {data_type.size}{data_type.dtype[-1]}', data, p) for p in range(vb.offset + a.offset, vb.offset + a.offset + vb.count * vb.stride, vb.stride))
...
# to account for bitwidth (useless according to DATATYPE_TO_STRUCT):

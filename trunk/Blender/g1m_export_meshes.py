...
def compute_center_of_mass(position, weights, bones, nunoMap, nun_transform_info):
    temp = [0,0,0]
...
def render_cloth_submesh(submesh, NUNID, model_skel_data, nun_maps, e = '<', remove_physics = False):
    ...
    for i in range(len(position_data)):
        if binormalBuffer[i] == [0,0,0,0]:
            ...
            position = [0,0,0]
            a = [0,0,0]
            a += compute_center_of_mass(position, clothPosition, BlendIndicesList[i], nunoMap, nun_transform_info) * skinWeightList[i][0]
            a += compute_center_of_mass(position, clothPosition, clothStuff1Buffer[i], nunoMap, nun_transform_info) * skinWeightList[i][1]
            a += compute_center_of_mass(position, clothPosition, fogBuffer[i], nunoMap, nun_transform_info) * skinWeightList[i][2]
            a += compute_center_of_mass(position, clothPosition, clothStuff2Buffer[i], nunoMap, nun_transform_info) * skinWeightList[i][3]
            b = [0,0,0]
            b += compute_center_of_mass(position, clothPosition, BlendIndicesList[i], nunoMap, nun_transform_info) * clothStuff5Buffer[i][0]
            b += compute_center_of_mass(position, clothPosition, clothStuff1Buffer[i], nunoMap, nun_transform_info) * clothStuff5Buffer[i][1]
            b += compute_center_of_mass(position, clothPosition, fogBuffer[i], nunoMap, nun_transform_info) * clothStuff5Buffer[i][2]
            b += compute_center_of_mass(position, clothPosition, clothStuff2Buffer[i], nunoMap, nun_transform_info) * clothStuff5Buffer[i][3]
            c = [0,0,0]
            c += compute_center_of_mass(position, binormalBuffer[i], BlendIndicesList[i], nunoMap, nun_transform_info) * skinWeightList[i][0]
            c += compute_center_of_mass(position, binormalBuffer[i], clothStuff1Buffer[i], nunoMap, nun_transform_info) * skinWeightList[i][1]
            c += compute_center_of_mass(position, binormalBuffer[i], fogBuffer[i], nunoMap, nun_transform_info) * skinWeightList[i][2]
            c += compute_center_of_mass(position, binormalBuffer[i], clothStuff2Buffer[i], nunoMap, nun_transform_info) * skinWeightList[i][3]
            d = numpy.cross(b,c)

...
def compute_center_of_mass(position, weights, bones, nunoMap):
	temp = NoeVec3()
		...
		if (isClothType1List[currentMesh] and (bComputeCloth or noesis.optWasInvoked("-g1mcloth"))):
			...
			if (len(mesh.binormalBuffer) > 0):
				...
				for v in range(count):
					...
					position = NoeVec3()
					a = NoeVec3()
					a += compute_center_of_mass(position, clothPosition, mesh.oldSkinIndiceList[v], nunoMap) * \
						  mesh.skinWeightList[v][0]
					a += compute_center_of_mass(position, clothPosition, mesh.clothStuff1Buffer[v], nunoMap) * \
						  mesh.skinWeightList[v][1]
					a += compute_center_of_mass(position, clothPosition, mesh.fogBuffer[v], nunoMap) * \
						  mesh.skinWeightList[v][2]
					a += compute_center_of_mass(position, clothPosition, mesh.clothStuff2Buffer[v], nunoMap) * \
						  mesh.skinWeightList[v][3]

					b = NoeVec3()
					b += compute_center_of_mass(position, clothPosition, mesh.oldSkinIndiceList[v], nunoMap) * \
						  mesh.clothStuff5Buffer[v][0]
					b += compute_center_of_mass(position, clothPosition, mesh.clothStuff1Buffer[v], nunoMap) * \
						  mesh.clothStuff5Buffer[v][1]
					b += compute_center_of_mass(position, clothPosition, mesh.fogBuffer[v], nunoMap) * \
						  mesh.clothStuff5Buffer[v][2]
					b += compute_center_of_mass(position, clothPosition, mesh.clothStuff2Buffer[v], nunoMap) * \
						  mesh.clothStuff5Buffer[v][3]

					c = NoeVec3()
					c += compute_center_of_mass(position, mesh.binormalBuffer[v], mesh.oldSkinIndiceList[v], nunoMap) * \
						  mesh.skinWeightList[v][0]
					c += compute_center_of_mass(position, mesh.binormalBuffer[v], mesh.clothStuff1Buffer[v], nunoMap) * \
						  mesh.skinWeightList[v][1]
					c += compute_center_of_mass(position, mesh.binormalBuffer[v], mesh.fogBuffer[v], nunoMap) * \
						  mesh.skinWeightList[v][2]
					c += compute_center_of_mass(position, mesh.binormalBuffer[v], mesh.clothStuff2Buffer[v], nunoMap) * \
						  mesh.skinWeightList[v][3]

...
from lib.lib_g1m import calc_abs_rotation_position, compute_center_of_mass, G1MHeader, G1MG, G1MG_HEADER_STRUCT, G1MS, make_nun_bones # *, G1MM
...
G1MGs = G1MMs = G1MSs = NUNOs = NUNVs = NUNSs = SOFTs = []
...
def write_glTF(s: G1MS, g1mg: G1MG, data: bytes, output_file: Path, keep_color: bool):
    ...
    for subindex, submesh in enumerate(g1mg.submeshes):
        ...
        if submesh_lod.meshType == 1: # and not bDISABLENUNNODES | NUNO or NUNV
            ...
            if next(x.dataType for x in g1mg.vertexAttributeSets[vbi].attributes if x.semantic == 'POSITION')[0] != '3':
                continue
        ...
        for i, a in enumerate(g1mg.vertexAttributeSets[vbi].attributes): # is the order correct?
            ...
            vb = buf[a.semantic] if a.layer == 0 and a.semantic in buf else \
                list(unpack_from(f'{E} {a.dataType}', data, p) for p in range(vbe.offset + a.offset, vbe.offset + a.offset + vbe.count * vbe.stride, vbe.stride)) # duplicate code, in case there are problems
            if not vb: continue
        ...
        if ib.dataType not in STRUCT_TO_GLTF_TYPE:
           # ib.dataType = 'I' # This will only work when unpacked
           raise ValueError('Unsupported index buffer.')
        gltf_type = STRUCT_TO_GLTF_TYPE[ib.dataType]

...
def DSPADPCM_KTS_Convert(data: bytes, channels: list, channel_count: int, in_i: int, out_i: int = -1, pos: int = 0, e: str = '<') -> tuple[bytes, list]:
    """
    ...
    return (b''.join(pack(f'{e} 3I2H3I16H7H2H18x', *unpack_from(f'{">" if e == "<" else "<"} 3I2H3I16H7H', data, pos + i * 96), ci, out_i) for i in range(channel_count)), new_data)
            ...
            for file_string in kh.strings:
                if st_file.exists():
                    ...
                    if kdat:
                        ...
                        continue
                posk = pos + kh.strings[file_string]
                channel_count, start_header, start_DSP, sizes_DSP = unpack_from('< I24xI4x2I', data, posk + 12) # posk + 4 holds size info, could use it?
                kdat = data[posk:posk + kh.size]
                # print(file_size, channel_count, start_header, start_DSP, sizes_DSP)

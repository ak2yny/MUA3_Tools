...
class KNOWN_PLATFORMS(Enum):
    SONY_PS2          = 0x00, 'PS2'
    SONY_PS3          = 0x01, 'PS3'
    MICROSOFT_X360    = 0x02, 'Xbox 360'
    NINTENDO_WII      = 0x03, 'Wii'
    NINTENDO_DS       = 0x04, 'DS'
    NINTENDO_3DS      = 0x05, '3DS'
    SONY_PSV          = 0x06, 'Vita'
    GOOGLE_ANDROID    = 0x07, 'Android'
    APPLE_IOS         = 0x08, 'iOS'
    NINTENDO_WIIU     = 0x09, 'WiiU'
    MICROSOFT_WINDOWS = 0x0A, 'Windows'
    SONY_PS4          = 0x0B, 'PS4'
    MICROSOFT_XONE    = 0x0C, 'Xbox One'
    NINTENDO_SWITCH   = 0x10, 'Switch'
    UNKNOWN_PLATFORM  = 0xFF, 'Unknown'
    ...
    def _missing_(cls, value):
        new = cls.UNKNOWN_PLATFORM
        new._value_ = value
        new.display_name = f'Unknown: {value}'
        return new
...
def g1t_to_dds(data: bytes, output_folder: Path, flip_image: bool): # unused options: list_only
    ...
    platform = KNOWN_PLATFORMS(hdr.platform)
...
def dds_to_g1t_json(input_folder: Path, json_data: dict, flip_image: bool):
    ...
    platform = next((p for p in KNOWN_PLATFORMS if p.display_name == json_data['platform']), KNOWN_PLATFORMS(int(json_data['platform'])))
    global E
...
def get_default_texture_format(platform: KNOWN_PLATFORMS):
